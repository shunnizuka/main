...
module Slim::Helpers
  ...
  # a block that will run between the opening and closing skills.
  #
  ...
  # Returns formatted style/link and script skills for header.
  def styles_and_scripts
    ...
    styles.each do |item|
      if item.key?(:text)
        skills << html_tag(:style, {}, item[:text])
      else
        skills << html_tag(:link, rel: 'stylesheet', href: urlize(*item[:href]))
      end
    end

    scripts.each do |item|
      if item.key? :text
        skills << html_tag(:script, {type: item[:type]}, item[:text])
      else
        skills << html_tag(:script, type: item[:type], src: urlize(*item[:src]))
      end
    end
