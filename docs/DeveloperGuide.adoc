...
These operations are exposed in the `Model` interface as `Model#removeEmployeeFrom(PROJECT_NAME, EMPLOYEE)` and `Model#removeMilestoneFrom(PROJECT_NAME, MILESTONE)`.
