...
This feature focuses on the adding of employees and milestones to a selected project and it is mainly facilitated
by the model component of the Pocket Project application. This feature supports two commands which are explained below.
...
These 2 commands are actually similar in nature and both of these are mainly facilitated
by the model component of the Pocket Project application. Both of these are supported by the
following methods in the `Model` interface:
...
`AddEmployeeToCommand#excute()`, `AddMilestoneToCommand#execute()` before the methods of `Model` are called.
