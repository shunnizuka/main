...
// skill::addTo[]
=== Add employee/milestone/project task/user story to projects feature
...
* `addto [project name] projecttask n/PROJECT_TASK_DESCRIPTION m/MILESTONE_INDEX` -- adds the project task with given description `PROJECT_TASK_DESCRIPTION` to the milestone specified by `MILESTONE_INDEX` in the project with name `[project name]`.
...
These operations are supported by the methods in the `Model` interface: `Model#addEmployeeTo(Project, Employee)`, `Model#addMilestoneTo(Project, Milestone)`, `Model#addProjectTaskTo(Project, Milestone, ProjectTask)` and `Model#addUserStoryTo(Projectm UserStory)`.
These four methods only take in the project and employee/milestone/project task/user story as arguments as the correct Employee/Milestone/ProjectTask/UserStory object would be located by
`AddEmployeeToCommand#excute()`, `AddMilestoneToCommand#execute()`, `AddProjectTaskToCommand#execute()` or `AddUserStoryToCommand#execute()` before the methods of `Model` are called.

Given below is an example usage scenario and how the addition of employee/milestone/project task/user story to a project feature behaves at each step.
...
`AddProjectTaskTo Command`

Step 1. The user enters the command `addto Apollo projecttask n/Create feature XYZ m/1`. The rest of the execution process is similar to Step 2 of the `AddEmployeeToCommand` mentioned above.
The only differences being that the object being added is a project task and that it is being added to a milestone within a project, instead of directly to the project.
...
4. `find employee` command is executed. Employees with name consisting the keywords are filtered out from the employeelist. The filtering is done using a predicate that checks through the names of the employee int the application.
...
* `stats PROJ_INDEX` or `stats PROJ_NAME` -- Display a summary of the progress of the project with the given PROJ_INDEX or PROJ_NAME.
...
1.	User executes `stats`/`stats PROJ_INDEX`/`stats PROJ_NAME` to view project statistics in Pocket Project.
...
* `addto PROJ_NAME userstory i/STORY_IMPORTANCE STORY` -- Adds the `story` to the project indicated by the `PROJ_NAME`.
* `removefrom PROJ_NAME userstory INDEX` -- Removes the user story at the specific index of the `SortedUserStoryList` in the project indicated by the `PROJ_NAME`.
* `edit project PROJ_NAME userstory INDEX STORY` -- Edits the user story at the specified index of the `SortedUserStoryList` in the project indicated by the `PROJ_NAME`.
* `update PROJ_NAME userstory INDEX STATUS -- Updates the status of the user story at the specified index of the `SortedUserStoryList` in the project indicated by the `PROJ_NAME`.
...
* `addto PROJ_NAME userstory [story]` -- <<Add employee/milestone/user story to projects feature>>
* `removefrom PROJ_NAME userstory INDEX` -- <<Remove employee/milestone from projects feature>>
...
=== Removing a milestone from a porject
