...
* `addto PROJECT_NAME userstory i/STORY_IMPORTANCE STORY` -- Adds the `story` to the project indicated by the `PROJECT_NAME`.
* `removefrom PROJECT_NAME userstory INDEX` -- Removes the user story at the specific index of the `SortedUserStoryList` in the project indicated by the `PROJECT_NAME`.
* `edit project PROJECT_NAME userstory INDEX STORY` -- Edits the user story at the specified index of the `SortedUserStoryList` in the project indicated by the `PROJECT_NAME`.
* `update PROJECT_NAME userstory INDEX STATUS -- Updates the status of the user story at the specified index of the `SortedUserStoryList` in the project indicated by the `PROJECT_NAME`.
...
* `addto PROJECT_NAME userstory [story]` -- <<Add employee/milestone/user story to projects feature>>
* `removefrom PROJECT_NAME userstory INDEX` -- <<Remove employee/milestone from projects feature>>
