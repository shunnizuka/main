...
This operation is supported by the `Project#UpdateStatus(Status)` method in the `Project` class as well as the `Model#getProjectWithName(Name)` method in the `Model` class.
