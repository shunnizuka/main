...
retain some flexibility so that the feature is more user-friendly. So, keeping that in mind, the dates used in the Pocket Project supports 2 main formats, the Fixed Date Format, which supports 1 main sub-type, and the Flexible Date Format,
which supports 3 main sub types.
...
Refer to the Activity Diagram above while reading the explanation so as to have a visual
representation of what happens.

To simplify the explanation, the process can be summarized into the following key points:

. A check is first made to determine if the input is a fixed or flexible date format. This is done by
checking for the presence of a `/` in the input.
. If it is a fixed format input, a check is made to determine if it conforms to the `DD/MM/YYYY` format.
. If it is a flexible format input, the input is parsed and the `PocketProjectParser` identifies keywords such
as `today/tomorrow/yesterday`, `this/next/last week` or `this,next,last month` to determine the appropriate methods to call from the model.
. Once the relevant model method is identified and the target date is calculated, it is formatted into
the `DD/MM/YYYY` format.
. Regardless of whether the input was fixed or flexible, the current string would represent the form
of `DD/MM/YYYY` at this stage.
. The date in the form `DD/MM/YYYY` is then checked to ensure that it is a valid calendar date.
...
The second design was chosen as the first one is ambiguous and subjective to an individual's language
...
it was designed such that users can key in the day number instead. This speeds things up and also reduces errors
...
. The `AddToCommandParser` then identifies the type of `AddToCommand` that is being executed and the arguments involved.
. The `AddToCommandParser` then creates the `AddEmployeeToCommand`/`AddMilestoneToCommand` object and this is passed on to the `LogicManager` for execution.
