...
retain some flexibility so that it is more user-friendly. So, the dates used in the Pocket Project supports 2 main formats, the Fixed Date Format, which supports 1 sub-type, and the Flexible Date Format,
which supports 3 sub-types.
...
. If it is a flexible format input, the input is parsed and the `PocketProjectParser` identifies keywords to determine the appropriate methods to call from the model. The target
date is then formatted into the DD/MM/YYYY format.
. Regardless of the initial format of the input, the current string would represent the form
...
use. For example, if today was Monday, and a person was to mention `next Wednesday`, some people would be thinking of the current week's Wednesday
whereas may be thinking of next week's Wednesday. Thus to
...
abstracted into out into a `AddToCommand` class which inherits from the `Command` class.
...
*Potential Design 2 (Selected Implementation)* +
Have a total of 2 parsers where the first parser, `PocketProjectParser` identifies the `addto` key word
