...
commands include the setting of start dates, deadlines and completion dates to projects and the setting of
...
|===
|image:ug-exclamation.png[width="40"]| The fixed date format follows the required `DD/MM/YYYY` format
strictly and so it requires `DD` for the day field and `MM` for the month field even if the value is only
single number. For example, `09/04/2023` is a valid input for the fixed date format but `9/4/2023` is not.
|===
...
a `s/START_DATE` and a `d/DEADLINE`. More details will be expounded on later in
//TODO link the shit
...
|===
|image:ug-exclamation.png[width="40"]|The `DAY_OF_MONTH` takes the value of positive integer but it cannot exceed
...
|===
|image:ug-lightbulb.png[width="40"]|The application will also automatically help to check the validity of the target
...
can be used to select the target `START_DATE` and `DEADLINE`.
|===
...
|===
|image:ug-lightbulb.png[width="50"]|Both the fixed date format and the flexible date format
can be used to select the target `MILESTONE_DATE`
|===
...
|===
|image:ug-notepadpencil.png[width="50"]| The list of milestones inside the target project
