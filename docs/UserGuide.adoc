...
[width="100%",cols="35%,<65%"]
|===
|image:ug-notepadpencil.png[width="40"]|The project has a default description when created and this can be
...
[width="100%",cols="35%,<65%"]
|===
|image:ug-lightbulb.png[width="50"]|Both the fixed and flexible date format can be used to
...
* `delete project 1` +
Deletes the first project shown in the most recent displayed list of projects. Figure 11 shows the command being typed but not entered yet
and figure 12 shows the effect of the command.
...
Below is the before and after screenshot to illustrate better.

.Before entering the command `edit employee 2 n/The magician p/98637251 s/`
image:edit_employee_before.png[]

.After entering the command `edit employee 2 n/The magician p/98637251 s/`
...
|===
|image:ug-exclamation.png[width="50"]|
Note that the deadline should be after the start date of the project and also after the date of the latest milestone in the project.
|image:ug-exclamation.png[width="50"]|
Note that project start date is not modifiable.
|===
...
.Before entering the `edit project Orbital milestone 1 m/Version 1.1 refactoring d/03/08/2019` command
image:edit_project_milestone_before.png[]

.After entering the `edit project Orbital milestone 1 m/Version 1.1 refactoring d/03/08/2019` command
...
.Afte entering `find employee stark` command
...
.Afte entering `find skill java` command
...
.Afte entering `find project apollo` command
...
.Afte entering `find all management` command
...
.Afte entering `find deadline this month 31` command
...
The employee at the specified `EMPLOYEE_INDEX` is added to the
project specified by the `PROJECT_NAME`.
...
[width="100%",cols="35%,<65%"]
|===
|image:ug-exclamation.png[width="60"]| As the milestone is part of a project's timeline, the
...
|===
|image:ug-exclamation.png[width="60"]| The `MILESTONE_INDEX` must be a positive integer and cannot be
...
|===
|image:ug-notepadpencil.png[width="50"]| Clicking on the arrow beside a milestone containing existing tasks allows you to hide the list of tasks under the milestone.
The arrow will only appear when there are one or more tasks in your project's milestone.
|===
...
|===
|image:ug-exclamation.png[width="60"]| The `MILESTONE_INDEX` and `PROJECT_TASK_INDEX` must be a positive integer and cannot be
...
|===
|image:ug-notepadpencil.png[width="50"]| The arrow beside the milestone description for collapsing the task list will disappear if all tasks for that milestone are removed.
|===
...
=== Edits UserStory of a project: `edit project userstory`

Allows the editing of the existing userstory in the userstory list of a project.

=== Login and encryption

We plan to implement a login feature so that only authenticated users would be able to access the app to modify and view the data stored.
However, unauthenticated users can still easily access the data through the JSON files that are used for storage in the current version, hence we
will also implement data encryption - which is elaborated upon in the next paragraph.

In the current app, the app data is stored in JSON format which is easily read and modified by anyone with access to the
JSON files. In V2.0, we intend to have commands that allow users to encrypt and decrypt the JSON files into an unreadable format.
It is also possible that we make encryption compulsory and by default, but that would be up for further discussion.

=== Sort the projects by name or deadline : `sort`

The following is the intended effect of this command:
...
We think it will good to have data visualization tools such as graphs and charts to show to the users the trends and status of the projects(such as
a graph showing how project progress changes as time progresses). This can help users to spot patterns that is otherwise hard to grasp if all the data
